...
using party_idx_t = coinbase::api::party_idx_t;
...
class job_mp_t {
 ...
 protected:
  ...
  // Transport lifetime:
  // - If the job is constructed with a `shared_ptr`, it keeps the transport alive.
  // - If the job is constructed with a `data_transport_i&`, it stores a non-owning
  //   pointer; the caller must ensure the transport outlives protocol execution.
  std::shared_ptr<coinbase::api::data_transport_i> transport_ptr;
  coinbase::api::data_transport_i* transport_raw = nullptr;

 public:
  job_mp_t(int index, std::vector<crypto::pname_t> pnames,
           std::shared_ptr<coinbase::api::data_transport_i> tptr = nullptr)
      : party_index(index),
        n_parties(pnames.size()),
        transport_ptr(std::move(tptr)),
  ...
  virtual error_t receive_impl(party_idx_t from, buf_t& msg) {
    if (!transport_raw) return E_NET_GENERAL;
    return transport_raw->receive(from, msg);
  }
  virtual error_t receive_many_impl(std::vector<party_idx_t> from_set, std::vector<buf_t>& outs);

  void set_transport(party_idx_t idx, std::shared_ptr<coinbase::api::data_transport_i> ptr) {
    party_index = idx;
    transport_ptr = std::move(ptr);
    transport_raw = transport_ptr.get();
  }
...
class job_2p_t : public job_mp_t {
 public:
  ...
  job_2p_t(party_t index, crypto::pname_t pname1, crypto::pname_t pname2, coinbase::api::data_transport_i& transport)
      : job_mp_t(party_idx_t(index), {pname1, pname2}, transport) {}
