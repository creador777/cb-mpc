#!/usr/bin/env bash
# build_fuzz_pve.sh — compila fuzz_pve.cpp contra cb-mpc (patron build_fuzz_schnorr.sh)
set -euo pipefail
...
cmake -S . -B build/fuzz -DCMAKE_C_COMPILER="$CC" -DCMAKE_CXX_COMPILER="$CXX" \
